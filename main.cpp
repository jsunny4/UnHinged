...
using namespace std;
int main() {
    PersonProfile joe("joe", "gmail");
    assert(joe.GetName() == "joe");
    assert(joe.GetEmail() == "gmail");
    AttValPair pair("hobby", "shitting");
    joe.AddAttValPair(pair);
    assert(joe.GetNumAttValPairs() == 1);
    AttValPair pair2;
    assert(joe.GetAttVal(0, pair2) == true);
    assert(pair2.attribute == "hobby" && pair2.value == "shitting");
    joe.AddAttValPair(AttValPair("hobby", "crying"));
    assert(joe.GetNumAttValPairs() == 2);
    assert(joe.GetAttVal(1, pair2) == true);
    joe.AddAttValPair(AttValPair("hobby", "crying and cumming"));
    assert(joe.GetNumAttValPairs() == 3);
    assert(joe.GetAttVal(1, pair2) == true);
    joe.AddAttValPair(AttValPair("food", "shit"));
    assert(joe.GetNumAttValPairs() == 4);
    for (int k = 0; k != joe.GetNumAttValPairs(); k++) {
     AttValPair av;
     joe.GetAttVal(k, av);
     std::cout << av.attribute << " -> " << av.value << std::endl;
     }
